...
import { handeldeleteMake } from "../../../../helper/server/services";
...
const ConfirmDeleteMake = ({ open, setOpen, id }: Props) => {
  const [token, setToken] = useRecoilState(TokenAtom);
  const [loadind, setLoading] = useState(false);

  const confirmDelete = async () => {
    setLoading(true);
    const res = await handeldeleteMake(token, id);
    if (res) {
...
export default ConfirmDeleteMake;
